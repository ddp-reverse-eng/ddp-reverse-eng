...
val wave : warn:(string -> unit) -> string -> t
(** A RIFF WAVE file at 44.1 kHz: PCM of 8, 16, 24 or 32 bits or float of 32 or
    64 bits, mono or stereo, plain or extensible header. Mono is reported
    through [warn]. *)
...
(** Checks that every sample converts exactly, reading the whole file when the
    encoding could fail.
    @raise Diag.Error naming the first sample that does not fit. *)
